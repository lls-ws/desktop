...
ytmusic_install()
{
	...
	sudo apt -y install playerctl xdotool wmctrl
...
ytmusic_config()
{
	...
	if [ ! -z "${USER}" ]; then
		...
		cat "${FILE_UPDATE}" | grep -E "startMinimized|windowMaximized|disableHardwareAcceleration|minimizeToTray|closeToTray"
